...
SERVER_URL = os.getenv("CI_SERVER_URL", "https://gitlab-master.nvidia.com")
...
def get_pipeline_jobs(
    pipeline_id: int, job_prefix: str
) -> list[tuple[str, int, list[dict]]]:
    """Collect jobs from Megatron's direct child pipelines."""
...
def configure_notification_urls() -> None:
    """Point triage notification links at the Megatron-LM project."""
...
@click.option(
    ...
    required=True,
    type=click.Choice(
        ["unit-tests", "integration-tests", "functional-tests", "smoke-tests"]
    ),
)
...
def main(pipeline_id: int, check_for: str, pipeline_context: str):
    if not WEBHOOK_URL:
    ...
    if check_for == "smoke-tests":
        if pipeline_jobs and all(
            job["status"] == "success" for _, _, jobs in pipeline_jobs for job in jobs
        ):
    ...
    slack_mentions = (
        f"{TEAM_SLUG} <!subteam^S0A7B4U1T3P> <@U09TX0DHZ97>" if TAG_TEAM else None
    )
