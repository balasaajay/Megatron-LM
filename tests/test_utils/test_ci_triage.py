...
def test_build_test_script_preserves_workload_exit_code():
    ...
    assert "set -e" in script
    assert "extract-errors jet_workload.log megatron_bridge" in script
    assert "--output error_report.json" in script
    assert 'exit "$exit_code"' in script


def test_build_test_script_runs_extractor_without_masking_failure(tmp_path):
    if shutil.which("extract-errors") is None:
        pytest.skip("nemo-ci-triage-2 is not installed")

    workload = 'import sys; print("RuntimeError: workload failed"); sys.exit(7)'
    command = f"{shlex.quote(sys.executable)} -c {shlex.quote(workload)}"
    script = generate_jet_trigger_job.build_test_script(command)

    result = subprocess.run(["bash", "-c", script], cwd=tmp_path, env=os.environ.copy())
    report = json.loads((tmp_path / "error_report.json").read_text())

    assert result.returncode == 7
    assert report["exit_code_training"] == 7
    assert report["category"] == "Python Error"


def test_get_pipeline_jobs_uses_triage_collector(monkeypatch, notify_module):
    notify = notify_module
    bridge = SimpleNamespace(
        name="functional:run_dev_dgx_h100",
        attributes={"downstream_pipeline": {"id": 101}},
    )
    root_pipeline = Mock()
...
def test_notification_delegates_to_triage_package(monkeypatch, notify_module):
    ...
    monkeypatch.setattr(
        notify, "PROJECT_URL", "https://ci.example.com/ADLR/megatron-lm"
    )
    ...
    assert (
        ...
        == "https://ci.example.com/ADLR/megatron-lm/-/jobs/{}"
    )
    assert (
